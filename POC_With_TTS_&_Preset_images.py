...
""")
            ...
            if st.session_state.active_source == f"preset_{name}":
                ...
                if st.button("Generate Caption", key=f"gen_{name}"):
                    with st.spinner("Generating caption..."):
                        st.session_state.active_caption = safe(
                            lambda: generate_caption(st.session_state.active_image)
                        )
    ...
    if uploaded:
        activate("upload", Image.open(uploaded).convert("RGB"))
    ...
    if use_camera:
        ...
        if cam:
            activate("camera", Image.open(cam).convert("RGB"))
...
• Caption appears in the same section  
""")
