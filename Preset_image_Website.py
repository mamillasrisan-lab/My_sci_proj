...
# TABS
# -----------------------------
gen_tab, processed_tab, helper_tab = st.tabs(
    ["Generate Caption", "Processed Images", "Helper"]
)
...
# -----------------------------
with gen_tab:
    st.write("Select a preset image, upload an image, use the camera, or provide an image URL.")
    ...
    try:
        if preset_choice != "None":
            r = requests.get(PRESET_IMAGES[preset_choice], timeout=10)
    ...
    if image:
        ...
        if st.button("Generate Caption"):
            ...
            st.success(caption)
            st.session_state.processed.append((image.copy(), caption))
...
with helper_tab:
    st.markdown("""
    **How this app works**
    - Choose a preset image, upload one, use your camera, or paste an image URL
    - Click **Generate Caption**
    - Captions are created using **BLIP-1**
    - All results are saved in the *Processed Images* tab
    """)
